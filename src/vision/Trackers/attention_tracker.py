...
class gazeTracker:
    ...
    def estimate_head_pose(self, landmarks, frame):
        ...
        # Note: Landmarks are from camera perspective (mirrored for webcams), so we
        # swap left/right indices to match the actual physical orientation.
        image_points = np.array(
            [
                (landmarks[1].x * w, landmarks[1].y * h),    # Nose tip
                (landmarks[152].x * w, landmarks[152].y * h), # Chin
                (landmarks[263].x * w, landmarks[263].y * h), # Right eye outer corner (appears left in mirrored view)
                (landmarks[33].x * w, landmarks[33].y * h),   # Left eye outer corner (appears right in mirrored view)
                (landmarks[291].x * w, landmarks[291].y * h), # Right mouth corner (appears left in mirrored view)
                (landmarks[61].x * w, landmarks[61].y * h),   # Left mouth corner (appears right in mirrored view)
            ],
            dtype="double",
        )
